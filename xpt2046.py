...
class Touch(object):
    ...
    def int_enabled(self, enabled):
        self.int_mutex.acquire()
        self.int_enabled = enabled
        self.int_mutex.release()
    ...
    def int_press(self, pin):
        ...
        if not self.int_enabled:
            logging.debug("Interrupt ignored (disabled)")
            self.int_mutex.release()
            return
        ...
        if not self.int_locked:
            ...
            buff = self.raw_touch()
            logging.debug(f"raw_touch returned: {buff}")

            if buff is not None:
                x, y = self.normalize(*buff)
                logging.debug(f"Normalized screen coordinates: ({x}, {y})")
    ...
    def int_release(self, pin):
        """Send X,Y values to passed interrupt handler."""
        if self.int_locked:
            logging.debug("Touch released, unlocking interrupt")
    ...
    def raw_touch(self):
        ...
        if x is None or y is None:
            logging.debug(f"raw_touch: SPI read failed (x={x}, y={y})")
            return None

        logging.debug(f"raw_touch: raw ADC values x={x}, y={y}  "
                      f"(valid range: x={self.x_min}-{self.x_max}, y={self.y_min}-{self.y_max})")

        if self.x_min <= x <= self.x_max and self.y_min <= y <= self.y_max:
            return (x, y)
        else:
            logging.debug(f"raw_touch: values OUT OF RANGE, returning None")
            return None
    ...
    def send_command(self, command):
        ...
        # 2. Lock the bus before using it
        while not self.spi.try_lock():
            sleep(0.01)  # Wait briefly before retrying to acquire the lock
            pass

        try:
            # Reconfigure SPI for the XPT2046 (may have been changed by
            # another device on the same bus, e.g. the ILI9341 display).
            self.spi.configure(baudrate=1_000_000, phase=0, polarity=0)

            self.cs.value = False

            logging.debug(f"Sent command: {[hex(b) for b in self.tx_buf]}")  # Debug print
            ...
            logging.debug(f"Received: {[hex(b) for b in self.rx_buf]}")  # Debug print

            return (self.rx_buf[1] << 4) | (self.rx_buf[2] >> 4)
        
        except Exception as e:
            self.cs.value = True  # Ensure CS is released on error
            self.spi.unlock()  # Ensure SPI is unlocked on error
            logging.error(f"SPI communication error: {e}")
            return None